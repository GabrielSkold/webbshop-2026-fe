    ...
    try {
        ...
        function renderProducts(products) {
            container.innerHTML = products.map(product => `
                <a class="product-href" href="product.html?id=${product._id}">
                    <article class="product-card">
                        <img class="product-card__image" src="${product.images[0].url}" alt="${product.name}" />
                        <div class="product-card__body">
                            <h3>${product.name}</h3>
        ...
        if(dropStatusParam) {
            const urlFiltered = products.filter(p => p.dropStatus === dropStatusParam);
        ...
        [...new Set(products.flatMap(p => p.sizes.map(s => s.size)))].forEach(size => {
            sizeFilter.innerHTML += `<option value="${size}">${size}</option>`;
        });

        [...new Set(products.flatMap(p => p.colors.map(s => s.name)))].forEach(color => {
        ...
        document.querySelector("#applyFilter").addEventListener("click", () => {
            const applyFiltered = products.filter(p => {
                ...
                if (sizeFilter.value && !p.sizes.some(c => c.size === sizeFilter.value)) return false;
                if (colorFilter.value && !p.colors.some(c => c.name === colorFilter.value)) return false;
                if (dropStatusFilter.value && p.dropStatus !== dropStatusFilter.value) return false;
                if (priceFilter.value && p.price > Number(priceFilter.value)) return false;
                if (stockFilter.value && String(p.inStock) !== stockFilter.value) return false;
                return true;
            });
